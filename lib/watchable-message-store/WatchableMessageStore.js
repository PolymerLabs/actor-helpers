...
export class WatchableMessageStore {
    constructor(name) {
        ...
        this.lastCursorId = 0;
        this.waitingRecipients = new Map();
        ...
        return new Promise((resolve, reject) => {
            ...
            connection.onupgradeneeded = () => {
                if (!connection.result.objectStoreNames.contains(this.objStoreName)) {
                    connection.result.createObjectStore(this.objStoreName, { autoIncrement: true });
                }
            };
        });
    }
     ...
     */
    async popMessages(recipient, { keepMessage = false, deleteImmediately = false } = {}) {
        return new Promise(async (resolve) => {
            const messages = [];
            const existingCallbacks = this.waitingRecipients.get(recipient) || [];
    ...
    async notifyAllRecipients(deleteImmediately = false) {
        return new Promise(async (resolve, reject) => {
            const transaction = (await this.database).transaction(this.objStoreName, 'readwrite');
            const cursorRequest = transaction.objectStore(this.objStoreName)
            ...
            cursorRequest.onsuccess = () => {
                ...
                if (cursor) {
                    ...
                    const waitingRecipientCallbacks = this.waitingRecipients.get(value.recipient);
                    ...
                    for (const waitingRecipientCallbacks of this.waitingRecipients
                        .values()) {
                        for (const waitingRecipient of waitingRecipientCallbacks) {
    ...
    async pushMessage(message) {
        ...
        return new Promise((resolve, reject) => {
            ...
            transaction.oncomplete = () => {
                if (this.bcc) {
                    this.bcc.postMessage({ recipient: message.recipient });
                }
                resolve();
            };
